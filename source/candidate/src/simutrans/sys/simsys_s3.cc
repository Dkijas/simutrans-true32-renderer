...
static bool internal_create_surfaces(int tex_width, int tex_height)
{
...
#if COLOUR_DEPTH == 32
	const SDL_PixelFormat pixel_format  = SDL_PIXELFORMAT_ARGB8888;
	...
	/* The format has to be exactly COLOUR_DEPTH bits with exactly the alpha
	 * mask the renderer expects - none at 16, 0xFF000000 at 32 - or the
	 * PIXVALs it writes mean something else on screen. Checked rather than
	 * assumed.
	 *
...
#if COLOUR_DEPTH == 32
	if(  details->bits_per_pixel != COLOUR_DEPTH  ||  details->Amask != expected_amask  ) {
		dbg->error( "internal_create_surfaces(SDL3)", "Pixel format error. Bpp got %d, needed %d. Amask got %u, needed %u.",
			details->bits_per_pixel, COLOUR_DEPTH, details->Amask, expected_amask );
		return false;
	}
#else

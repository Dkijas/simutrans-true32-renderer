...
#if COLOUR_DEPTH == 32
	// the framebuffer is ARGB8888; BI_RGB at 32 bpp describes exactly that
	// (validated against the DIB masks with a standalone control). The
	// RGB565 bit fields below would make GDI read 16 bit masks out of
	// 32 bit words.
	header.biCompression   = BI_RGB;
	masks[0]               = 0;
	masks[1]               = 0;
...
{
#if COLOUR_DEPTH == 32
	// full ARGB8888: the screen colour keeps every channel bit
				...
				if(  gui_component_t *c = win_get_focus()  ) {
					if(  gui_textinput_t *tinp = dynamic_cast<gui_textinput_t *>(c)  ) {
						...
						for(  DWORD i=0; i<icp->dwCharPos; ++i  ) {
							...
							gfx->get_next_char_with_metrics( composition, unused, pixel_width );
							x += pixel_width;
						}
						icp->pt.x = x;

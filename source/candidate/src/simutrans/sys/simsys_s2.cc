...
{
	// The pixel format has to match the graphics code: RGB565 for
	// simgraph16.cc, ARGB8888 for simgraph32.cc. Alpha inside the
	// framebuffer is handled by the renderer, not by SDL - but ARGB8888
	// carries an alpha mask and the check below must expect it.
#if COLOUR_DEPTH == 32
	const Uint32 pixel_format  = SDL_PIXELFORMAT_ARGB8888;
	const Uint32 expected_amask = 0xFF000000u;
#else
	...
	// Color component bitmasks for the pixel format the renderer writes
	int bpp;
...
#if COLOUR_DEPTH == 32
	else if(  bpp != COLOUR_DEPTH  ||  amask != expected_amask  ) {
		dbg->error( "internal_create_surfaces(SDL2)", "Pixel format error. Bpp got %d, needed %d. Amask got %u, needed %u.", bpp, COLOUR_DEPTH, amask, expected_amask );
		return false;
	}
#else
...
// resizes screen
int dr_textur_resize(PIXVAL** const textur, int tex_w, int const tex_h)
{
...
{
#if COLOUR_DEPTH == 32
	// The framebuffer is the surface; dr_flush() uploads it with
	// SDL_UpdateTexture. Holding a lock on the streaming texture meanwhile
	// leaves D3D9 showing a black ARGB8888 texture, so no lock is taken.
...
{
#if COLOUR_DEPTH == 32
	// full ARGB8888: the screen colour keeps every channel bit, exactly as
	// simsys_w.cc computes it, so all backends agree bit for bit
	return 0xFF000000u | ((uint32)col.r << 16) | ((uint32)col.g << 8) | (uint32)col.b;
#else
...
static inline unsigned int ModifierKeys()
{
	...
	return
		  ((mod & KMOD_SHIFT) ? SIM_KEYMOD_SHIFT : SIM_KEYMOD_NONE)
		| ((mod & KMOD_CTRL)  ? SIM_KEYMOD_CTRL  : SIM_KEYMOD_NONE)
#ifdef __APPLE__
		// Treat the Command key as a control key.
		| ((mod & KMOD_GUI)   ? SIM_KEYMOD_CTRL  : SIM_KEYMOD_NONE)
#endif
		;
}

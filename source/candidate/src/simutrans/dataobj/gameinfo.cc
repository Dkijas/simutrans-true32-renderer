...
gameinfo_t::gameinfo_t(karte_t *welt) :
	map_pixval(MINIMAP_SIZE,MINIMAP_SIZE),
	game_comment(""),
	file_name(""),
	pak_name("")
{
	...
	for( uint16 i = 0; i < MINIMAP_SIZE; i++ ) {
		for( uint16 j = 0; j < MINIMAP_SIZE; j++ ) {
			...
			map_pixval.at(i,j) = minimap_t::calc_ground_color(gr);
		}
	}
...
void gameinfo_t::rdwr(loadsave_t *file)
{
	...
	for( int y=0;  y<MINIMAP_SIZE;  y++  ) {
		for( int x=0;  x<MINIMAP_SIZE;  x++  ) {
			...
			NETWORK_PIXVAL c = network_colour_from_screen( map_pixval.at(x,y) );
			file->rdwr_short( c );
			map_pixval.at(x,y) = screen_colour_from_network( c );
		}
	}

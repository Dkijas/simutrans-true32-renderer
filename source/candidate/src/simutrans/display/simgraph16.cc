...
{
	clip_core_init_ranges( y, CR.number_of_clips, CR.active_ribi,
	                       CR.clip_ribi, CR.poly_clips, CR.xranges );
}
...
inline void get_xrange_and_step_y(int &xmin, int &xmax  CLIP_NUM_DEF)
{
	...
	clip_core_step_y( xmin, xmax, CR.number_of_clips, CR.active_ribi,
	                  CR.clip_ribi, CR.poly_clips, CR.xranges );
}
...
static void rezoom_img(const image_id n)
{
	...
	if(  n < anz_images  &&  images[n].base_h > 0  ) {
		...
		// the resampling itself is renderer-independent; this renderer keeps the
		// historical in-band clear marker, so its output does not move
		zoom_scratch_t scratch;
		scratch.unpack   = rezoom_baseimage[n % env_t::num_threads];
		scratch.resample = rezoom_baseimage2[n % env_t::num_threads];
		scratch.size     = rezoom_size[n % env_t::num_threads];
		...
		zoom_core_rezoom<zoom_sample_inband_t>(
			images[n].base_data,
			images[n].base_x, images[n].base_y, images[n].base_w, images[n].base_h,
			g_simgraph16.zoom_num[zoom_factor], g_simgraph16.zoom_den[zoom_factor],
			scratch, out );

		rezoom_baseimage[n % env_t::num_threads]  = scratch.unpack;
		rezoom_baseimage2[n % env_t::num_threads] = (PIXVAL *)scratch.resample;
		rezoom_size[n % env_t::num_threads]       = scratch.size;
...
/**
 * Pixel policy of the CLIPPED 16 bit blitters.
 *
 * Deliberately a different policy from PixelOps16, which nc uses. wc and pc
 * have always reached the plain pixcopy(), never the unrolled
 * two-pixels-per-uint32 copy; that asymmetry is historical, and this cut
 * preserves it instead of tidying it away. Keeping the unrolled copy in
 * PixelOps16 and out of this one is what makes the difference impossible to
 * lose by accident.
 *
 * The source space is the routine's own: plain reads a recoded screen cache,
 * colored and daytime read the original stored image data.
 */
template<pixcopy_routines copyroutine>
struct PixelOps16c {
	typedef PIXVAL                   screen_t;   ///< RGB565
	typedef pixcopy_src<copyroutine> source_t;

	static inline void copy(screen_t *dest, const source_t *src, const source_t * const end)
	{
		templated_pixcopy<copyroutine>(dest, src, end);
	}

	static inline void alpha(screen_t *dest, const source_t *src, const source_t * const end)
	{
		templated_alphacopy<copyroutine>(dest, src, end);
	}
};


/**
 * Row interval of the polygon-clipped blitter.
 *
 * It carries the clip number and nothing else: the clipping state itself stays
 * in CR, where it has always been, and this only asks it for the next row.
 */
struct span_poly16_t {
#ifdef MULTI_THREAD
	const sint8 clip_num;
#endif

	inline void next(int &xmin, int &xmax) const { get_xrange_and_step_y( xmin, xmax  CLIP_NUM_PAR ); }
};
...
static void display_img_pc(scr_coord_val h, const scr_coord_val xp, const scr_coord_val yp, const pixcopy_src<copyroutine> *sp  CLIP_NUM_DEF)
{
	if(  h > 0  ) {
		...
		blit_core_clipped<PixelOps16c<copyroutine> >( h, textur + yp * disp_width, sp, xp, disp_width,
		                                             span_poly16_t{ CLIP_NUM_VAR } );
	}
}
...
{
	if(  h > 0  ) {
		blit_core_clipped<PixelOps16c<plain> >( h, textur + yp * disp_width, sp, xp, disp_width,
		                                        span_fixed_t{ CR.clip_rect.x, CR.clip_rect.xx } );
	}
}


/**
 * Pixel policy of the 16 bit renderer.
 *
 * Note the copy: the unrolled two-pixels-per-uint32 path only makes sense
 * while a screen pixel is 16 bit, so this is where it belongs. It is moved
 * here verbatim from display_img_nc; wc and pc keep calling pixcopy() as they
 * always did, and whether they should share this copy is a question for the
 * cut that migrates them.
 */
struct PixelOps16 {
	typedef PIXVAL screen_t;   ///< RGB565
	...
	static inline void copy(screen_t *dest, const screen_t *src, const screen_t * const end)
	{
		...
		size_t runlen = (size_t)(end - src);
		if(  runlen  ) {
			// align to 4 bytes, should use uintptr_t but not available
			if(  reinterpret_cast<size_t>(dest) & 0x2  ) {
				*dest++ = *src++;
				runlen--;
			}
			...
			runlen >>= 1;
			uint32 *ld = reinterpret_cast<uint32 *>(dest);
			while(  runlen--  ) {
#if defined _MSC_VER // MSVC can read unaligned
				*ld++ = *(uint32 const *const)src;
#else
				// little endian order, assumed by default
				*ld++ = (uint32(src[1]) << 16) | uint32(src[0]);
#endif
				src += 2;
			}
			dest = (screen_t*)ld;
...
{
	blit_core_nc<PixelOps16>( h, textur + xp + yp * disp_width, sp, disp_width );
}

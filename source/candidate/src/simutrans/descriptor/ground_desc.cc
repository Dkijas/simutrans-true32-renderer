...
#if COLOUR_DEPTH != 0
	STORED_PIXVAL* dest = image_dest->get_data();

	STORED_PIXVAL const* const texture = image_texture->get_data();
...
static image_t* create_alpha_tile(const image_t* image_lightmap, slope_t::type slope, const image_t* image_alphamap)
{
	...
	STORED_PIXVAL const* const alphamap  = image_alphamap->get_data();
...
static image_t* create_texture_from_tile(const image_t* image, const image_t* ref)
{
	...
	STORED_PIXVAL *const sp2 = image_dest->get_data();
	...
	const STORED_PIXVAL* sp = image->get_data();
